            ...
            dbc.ModalBody([
                ...
                dbc.Row([
                    ...
                    dbc.Col([
                        ...
                        html.Div('',id='existe-modelo3'),
                        dbc.Button('Verificar', id='verificar_modelo', size='md',className='btn btn-primary mt-3'), #className='btn btn-primary mt-3',style={'height': '40%'}),
                        dbc.Label("       "),
                        dbc.Button('Excluir', id='excluir_modelo', size='md',className='btn btn-primary mt-3', color="danger")
                        ], sm=12, md=4)
...
def tags_tabela(btn, dados,area,tag,ds_modelo):
        ...
        if id_modelo != None:
            ...
            cursor.execute("INSERT INTO TB_MODELO (ID_MODELO, ID_AREA, MODELO, DS_MODELO, ATIVO) VALUES (newid(),?, ?, ?,?)", (area, tag, ds_modelo,0))
            #cnxn.commit()
...
@app.callback(Output('existe-modelo3', 'children'),
              Input('excluir_modelo','n_clicks'))
...
def Excluir(btn):
    ...
    if id_modelo != None:
            ...
            cursor.execute("DELETE FROM TB_MODELO where ID_MODELO='"+ id_modelo+ "'")
            cursor.execute("DELETE FROM TB_CUTOVER where MODELO='"+ id_modelo+ "'")
            cursor.execute("DELETE FROM TB_TAGS where MODELO='"+ id_modelo+ "'")
            cursor.execute("DELETE FROM TB_Anomalias where MODELO='"+ id_modelo+ "'")
            cursor.execute("DELETE FROM TB_TRAIN where MODELO='"+ id_modelo+ "'")
            cnxn.commit()
